...
class BaseDataSets(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        h5f = cv2.imread('../data/fundus/{}.png'.format(idx))
        h5f=cv2.cvtColor(h5f,cv2.COLOR_RGB2GRAY)
...
class RandomGenerator(object):
    ...
    def __call__(self, sample):
        ...
        label3 = zoom(label, (self.output_size[0] / x, self.output_size[1] / y), order=0)
        label0 = zoom(label, (28/ x, 28/ y), order=0)
        label1 = zoom(label, (56 / x, 56 / y), order=0)
        label2 = zoom(label, (112/ x, 112 / y), order=0)

        image = torch.from_numpy(image.astype(np.float32)).unsqueeze(0)
        image=image/255
        label = torch.from_numpy(label.astype(np.uint8))

        sample = {"image": image, "label3": label3,'label0':label0,'label1':label1,'label2':label2}
        return sample
